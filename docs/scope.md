...
| **Any "auto" model selection** | Picking a model from guessed task difficulty needs either a classifier call on the critical path — latency and cost on every turn, against the sub-second first token — or a capability ranking we maintain by hand, which is the release-per-model-launch coupling design §10.2 exists to remove. models.dev has no quality field to answer it from. Worse, a model switch invalidates the entire prompt cache, so routing *down* to save money can cost several times more for that step. pi declines to route at all — its `auto` is a model id forwarded to a remote gateway, and it classifies nothing. **`/model` is the whole user-facing story.** What we do instead: never validate model ids against the catalog, so `openrouter/auto` and any future router work untouched; a default model per provider; and one internal key for the cheap model behind compaction and titles (internals §6.2), which is not routing but a refusal to run a flagship on a summarization job. See findings. **Nothing user-facing may be named `auto`** — that is a permission mode (design §7.2), and the status line shows mode and model side by side |
