...
| **Difficulty-based model routing** | Picking a model from guessed task difficulty needs either a classifier call on the critical path — latency and cost on every turn, against the sub-second first token — or a capability ranking we maintain by hand, which is the release-per-model-launch coupling design §10.2 exists to remove. models.dev has no quality field to answer it from. Worse, a model switch invalidates the entire prompt cache, so routing *down* to save money can cost several times more for that step. `openrouter/auto` gives users the feature without us shipping a router; roles by job (`main`/`small`/`plan`, internals §6.2) give the deterministic part. See findings. **A model selector must never be called `auto`** — that is a permission mode (design §7.2), and the status line shows mode and model side by side |
