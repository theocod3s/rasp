...
Seven built-in tools, one config file, a handful of slash commands. pi is a genuinely capable
agent with exactly seven tools. neo's config surface has already churned once
(`permissions` → `tool_approvals`) and it's a young project. Every knob is a compatibility
commitment. MCP is the pressure valve — users add tools without us adding surface.
...
- **WON'T** fetch a remote model catalog in v1. Hardcoded per-provider defaults plus
  user-defined models in config.

### 6.2 Tools

- **MUST** ship exactly seven **built-in** tools: `read`, `write`, `edit`, `bash`, `grep`,
  `find`, `ls`. Users extend the set through MCP (§6.8), not by us adding more.
...
- **SHOULD** serialize concurrent mutations to the same file via a per-path lock.
- **WON'T** implement `multiedit`, `todos`, `web_fetch`, `web_search` or a sub-agent tool in v1.
...
- **WON'T** use a database in v1. Add an index only when listing or search actually needs one.
...
- **SHOULD** read Claude Desktop's config location as an additional discovery source, so
  existing setups work with zero configuration.
...
No config file, no wizard. `rasp` reads `ANTHROPIC_API_KEY` from the environment and starts.
Configuration is a single JSON file, discovered per-project then global:
...
| **S11** | Plan mode cannot modify the filesystem. Not "doesn't by convention" — cannot | Automated: run a scripted session in plan mode against a git-clean tree, assert `git status` is still clean afterward |
...
stop reason. All seven tools. The correctness invariants from §6.6 — pairing, truncation
guard, panic recovery, loop detection. Workspace confinement. Headless mode only.
...
**Deliberately last.** stdio transport via the official Go SDK. Discovery from `.mcp.json`,
rasp config, and optionally Claude Desktop's location. Namespaced merge into the tool registry.
The three guardrails: tool-count budget with per-server allow-list, lazy connect with a hard
timeout, and failures surfaced as ordinary tool errors.
...
| **R4** | **Scope creep.** Every reference project has features rasp won't, and each looks small in isolation. Crush is 145k lines and grew a client/server split, a hooks engine, three OAuth flows and a Bash config DSL | §3's non-goals are decisions, not a backlog. Anything not in [scope.md](scope.md)'s MVP section requires explicitly moving it there first. The seven-tool limit is the canary — if it grows during M1, scope is slipping |
...
Genuinely undecided. Each needs resolving before the milestone that depends on it.

**Q1. How aggressive should parallel tool execution be?** *(needed by M1)*
neo runs up to 8 concurrently; pi is parallel by default; **Crush marks only the sub-agent tool
parallel and runs everything else sequentially.**
→ *Recommendation:* sequential in v1, following Crush. It's easy to reason about, parallel file
reads are rarely the bottleneck, and it removes a class of ordering bug during the milestone
where the loop is being learned. Revisit when sub-agents land.

**Q2. Hardcoded model list, or fetched catalog?** *(needed by M3)*
pi generates one from models.dev; Crush fetches its own `catwalk` service with ETag caching;
neo hardcodes and live-fetches only for OpenRouter. Both mature projects fetch.
→ *Recommendation:* hardcoded defaults plus user-defined models in config for v1. Fetching
introduces a network dependency on startup and a third-party contract, for a benefit
(discovering new model IDs automatically) that matters at their scale, not ours.

**Q3. Does a `todos` tool belong in v1?** *(needed by M1)*
Both neo and Crush ship one, and it visibly improves multi-step work by making the model's plan
inspectable. It would make eight tools, breaking the "pi's seven exactly" line.
→ *Recommendation:* no for v1, revisit immediately after M1. The seven-tool limit is doing
useful work as a scope canary (R4), and this is the most defensible thing to add first once
the core is proven.

**Q4. Config format — JSON, TOML or YAML?** *(needed by M0)*
JSON has no comments, which is genuinely annoying in a config file. TOML and YAML both add a
dependency; YAML adds ambiguity.
→ *Recommendation:* JSON with `//` comments stripped before parsing (JSONC), matching what
Crush and opencode both use. Familiar, one small dependency, comments work.

**Q5. When does a SQLite session index become necessary?** *(deferred past v1)*
JSONL alone means listing sessions requires reading every file's head.
→ *Recommendation:* defer until it's actually slow. Claude Code uses plain JSONL at
considerable scale. Revisit if the session picker takes more than ~50ms.

**Q6. What is the system prompt, actually?** *(needed by M1)*
Not a technical question but a product one, and it disproportionately determines quality.
opencode maintains six model-family-specific prompts; Crush uses Go templates; neo hardcodes a
~25-line string.
→ *Recommendation:* start with one short prompt, embedded and version-controlled, and treat it
as a tuning surface with the golden edit corpus (S4) as the feedback signal. Resist per-model
variants until there's evidence a specific model needs one.

**Q7. Should rasp read Claude Desktop's MCP config?** *(needed by M4)*
Marked SHOULD in §6.8. Reading
`~/Library/Application Support/Claude/claude_desktop_config.json` and its platform equivalents
gives genuine zero-config interop for anyone already using MCP. The cost is a platform-specific
path table plus inheriting another product's schema, which can change under us.
→ *Recommendation:* yes, but read-only, best-effort, and silent on failure — never a startup
error. Project `.mcp.json` and rasp's own config win on a name collision.

**Q8. Curated or minimal allow-list for plan mode's bash?** *(needed by M2)*
opencode hand-curates one (`git diff*`, `git log*`, `git status*`, `find *`, with
`find * -delete*` and `find * -exec*` carved back out to `ask`). Curation is precise but never
complete; the alternative is denying bash outright in plan mode.
→ *Recommendation:* start from opencode's curated list — it's been through real use — and treat
gaps as bug reports. Denying bash entirely would gut plan mode, since reading `git log` is
exactly what planning needs.

**Q9. Does the mode belong in the session file or in config?** *(needed by M3)*
§6.7 requires mode to persist per session. But a user who sets `"mode": "plan"` globally
presumably wants new sessions to start there.
→ *Recommendation:* config sets the mode for *new* sessions; the session file records the
active mode and wins on resume. A resumed plan-mode session should not silently become
auto because the global default changed.
